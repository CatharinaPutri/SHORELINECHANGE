"""
==========================================================
  DSAS-style Shoreline Change Analysis Module
...
def run_dsas_analysis(
    ...
    if not shoreline_paths or len(shoreline_paths) < 2:
        raise ValueError("DSAS analysis requires at least two shoreline datasets.")
    ...
    for y in years:
        ...
        shore_data[y] = _load_shoreline(path, year=y)

    crs = shore_data[ref_year]["crs"]
    _validate_common_crs(shore_data)
    ...
    baselines = _build_baselines(
        ref_segments,
        offset=p["baseline_offset"],
        placement=p["baseline_placement"],
        orientation=p["baseline_orientation"],
        min_length=p["min_baseline_length"],
    )
    if not baselines:
        raise ValueError("No baseline segments could be generated from the reference shoreline.")

    baseline_gdf = gpd.GeoDataFrame(baselines, geometry="geometry", crs=crs)
    transects_gdf = _cast_transects(
        baseline_gdf,
        spacing=p["transect_spacing"],
        length=p["transect_length"],
        placement=p["baseline_placement"],
        orientation=p["baseline_orientation"],
    )
    if transects_gdf.empty:
        raise ValueError("No transects were generated. Check spacing/length/baseline settings.")
    ...
    intersections_df, _, diagnostics_df = _intersect_transects(
        ...
        require_all=p["require_all_years"],
    )

    stats_df = _compute_stats(intersections_df, p["highest_unc"], p["epr_unc"])

    transects_path = os.path.join(output_dir, "transects.geojson")
    transects_gdf.to_file(transects_path, driver="GeoJSON")

    diagnostics_csv_path = os.path.join(output_dir, "intersection_diagnostics.csv")
    diagnostics_df.to_csv(diagnostics_csv_path, index=False)

    stats_csv_path = os.path.join(output_dir, "dsas_stats.csv")
    stats_df.to_csv(stats_csv_path, index=False)

    stats_geojson_path = os.path.join(output_dir, "dsas_stats.geojson")
    if not stats_df.empty:
        stats_geo = transects_gdf[["transect_id", "geometry"]].merge(
            stats_df, on="transect_id", how="inner"
        )
        stats_gdf = gpd.GeoDataFrame(stats_geo, geometry="geometry", crs=crs)
        stats_gdf.to_file(stats_geojson_path, driver="GeoJSON")
    else:
...
def _validate_common_crs(shore_data: dict) -> None:
    crs_values = {str(v["crs"]) for v in shore_data.values()}
    if len(crs_values) != 1:
        raise ValueError("All shoreline files must use the same CRS.")

    sample_crs = next(iter(shore_data.values()))["crs"]
    if sample_crs is None:
        raise ValueError("Shoreline CRS is missing.")

    if hasattr(sample_crs, "is_geographic") and sample_crs.is_geographic:
        raise ValueError(
            "Shoreline CRS is geographic (degrees). Reproject to a projected CRS in meters first."
        )
...
def _load_shoreline(path, year=None):
    ...
    if not raw:
        return {"year": year, "segments": [], "crs": gdf.crs}
    ...
    segments = [_orient_west_to_east(seg) for seg in segments if seg.length > 0]
    return {"year": year, "segments": segments, "crs": gdf.crs}
...
def _compute_stats(intersections_df, highest_unc=5.0, epr_unc=1.0):
    if intersections_df.empty:
        return pd.DataFrame(
            columns=[
                "transect_id", "baseline_id", "reference_segment_id", "dist_on_baseline",
                "oldest_year", "newest_year", "n_years", "SCE", "SCE_trend",
                "NSM", "NSM_trend", "EPR", "EPR_trend", "LRR"
            ]
        )
